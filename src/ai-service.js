...
function safetyCheck(question) {
  ...
  const RELEVANT_KEYWORDS = /运势|命理|八字|星座|塔罗|梅花|占卜|五行|命盘|排盘|性格|感情|恋爱|婚姻|结婚|事业|工作|财运|健康|学业|考试|投资|运气|前途|未来|今年|明年|本月|下半年|桃花|姻缘|合婚|配对|合盘|合适|般配|大运|流年|吉凶|方位|幸运|适合|注意|建议|怎么样|如何|好不好|能不能|会不会|什么时候|跳槽|升职|创业|买房|卖房|房子|租房|搬家|转行|出国|留学|分手|复合|怀孕|减肥|生意|发财|破财|人生|命运|天赋|潜力|弱点|优势|缺点|身体|手术|脾气|个性|缘分|伴侣|对象|另一半|属相|生肖|日主|天干|地支|纳音|格局|十神|宫位|行星|月亮星座|太阳星座|上升|水逆|卦|爻|易经|风水|方向|数字|时辰|农历|阴历|吠陀|印度占|该不该|要不要|值不值|选择|决定|纠结/;
...
function buildRequest(mode, profile, question, options = {}) {
  // 安全检查 (梅花易数跳过相关性检测——一事一占，什么都能问)
  const safety = safetyCheck(question);
  if (!safety.safe && !(mode === 'meihua' && safety.level === 'irrelevant')) {
