...
**代码:** 单文件架构 `app.html` (~5200行) + `test-server.js` (~700行)
...
1. **深色模式**: 大部分已修复, 隐私设置弹窗内部分颜色仍硬编码
2. **缘友圈**: 方案A(纯社区), "想认识"需后端
3. **Bot聊天**: 使用定时器模拟, 非真实WebSocket
4. **数据**: 全部localStorage, 无云端同步
5. **性能**: app.html单文件5200行, 生产应拆分+构建
...
| 任务 | 时间 | 状态 |
|------|------|------|
| 技术选型(Node/Python, DB) | 1天 | ⬜ |
| 数据库设计(PostgreSQL/MongoDB) | 2天 | ⬜ |
| 用户注册/登录API | 3天 | ⬜ |
| JWT认证中间件 | 1天 | ⬜ |
| 用户资料CRUD | 2天 | ⬜ |
| 占卜历史云端存储 | 2天 | ⬜ |
